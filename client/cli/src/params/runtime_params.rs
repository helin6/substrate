...
pub struct RuntimeParams {
	/// The size of the instances cache for each runtime. The values higher than 1000 are illegal.
...
fn parse_max_runtime_instances(s: &str) -> Result<usize, String> {
	...
	if max_runtime_instances > 1000 {
		Err(format!("Illegal `--max-runtime-instances` value: {max_runtime_instances} is more than the allowed maximum of `1000` "))
	} else {
